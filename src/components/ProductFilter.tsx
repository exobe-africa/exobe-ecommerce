...
interface FilterProps {
  ...
  // Category filter (optional - only for search page)
  ...
  return (
    <div className={`${isMobile ? 'fixed inset-0 bg-white z-50 overflow-y-auto' : 'sticky top-4'}`}>
      ...
      <div className={`space-y-6 ${isMobile ? 'p-4' : ''}`}>
        ...
        {/* Categories Filter (only for search page) */}
        ...
        <div>
          ...
          {brands.length > 5 ? (
            // Dropdown for many brands (category page style)
            <select
            ...
            // Checkboxes for fewer brands (search page style)
            <div className="space-y-2">
              {brands.map(brand => (
